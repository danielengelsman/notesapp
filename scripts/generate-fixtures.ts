...
// demo-data module bundled into the app (so the in-app demo needs no fetch)
{
  const { readFileSync } = require("node:fs") as typeof import("node:fs");
  const esc = (s: string) => s.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$\{/g, "\\${");
  const mod = `/** AUTO-GENERATED by scripts/generate-fixtures.ts — do not edit.
...
export const DEMO_IIF = \`${esc(readFileSync(join(OUT, "lists.iif"), "utf8"))}\`;
export const DEMO_TXN_CSV = \`${esc(readFileSync(join(OUT, "txn-detail.csv"), "utf8"))}\`;
export const DEMO_TB_CSV = \`${esc(readFileSync(join(OUT, "trial-balance-2025.csv"), "utf8"))}\`;
`;
  mkdirSync(join(process.cwd(), "src", "app", "app"), { recursive: true });
  writeFileSync(join(process.cwd(), "src", "app", "app", "demo-data.ts"), mod);
}
